...
import numpy as np
from modelsAE.cifar import autoencoder
...
flags.DEFINE_multi_integer('ch_mult', [1, 2, 2, 4], help='channel multiplier')
...
flags.DEFINE_integer('total_steps', 200000, help='total training steps')
...
flags.DEFINE_bool('resume_training', True, help='resume training') 
# Logging & Sampling
flags.DEFINE_string('logdir', './DMAE_cifar10-2-100/DiffusionAEOT', help='log directory')############################### --eval modified
...
flags.DEFINE_string('AE_OT_root', './cifar10-2/', help='features of AE')
flags.DEFINE_integer('dim_z', 512, help='dimension of feature in latent space')
flags.DEFINE_integer('dim_c', 3, help='input image number of channels')
flags.DEFINE_integer('dim_f', 64, help='number of features in first layer of AE')
flags.DEFINE_integer('sampling_ckpt_id', 140000, help='selected the best ckpt for sampling')############################### --eval modified
...
def evaluate(sampler,DiffusionStep,  model, features, AE):
    ...
    np.savez("./gen_cifar-{}.npz".format(FLAGS.truncated_T), arr_0=images)
    print(images.shape)  
    end = time.time()
